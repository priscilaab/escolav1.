...
class Docente{
    ...
    public function Inserir(){
        //$sql = "INSERT INTO docente (nome_docente, email_docente, especializacao_docente,telefone_docente) VALUES(?,?,?,?)";
        $sql = "CALL cad_docente(?,?,?,?,?)";
        ...
        $comando->execute(array($this->nome, $this->email, $this->especializacao, $this->tel1, $this->tel2));
    ...
    public function Atualizar(){
        $banco = Banco::conectar();
        $sql = "UPDATE docente SET nome = ?, email = ?, especializacao = ? WHERE id = ?";
        $banco->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $comando = $banco->prepare($sql);
